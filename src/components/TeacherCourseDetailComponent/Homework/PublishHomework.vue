<template>
    <div class="homework-container">
      <!-- 发布作业表单 -->
      <el-form :model="form" ref="form" label-width="120px" class="homework-form" @submit.prevent="submitHomework">
        <!-- 作业编号 -->
        <el-form-item label="作业编号" prop="homeworkNum" :rules="[{ required: true, message: '请输入作业编号', trigger: 'blur' }]">
          <el-input v-model="form.homeworkNum" type="number" placeholder="请输入作业编号"></el-input>
        </el-form-item>
  
        <!-- 截止时间 -->
        <el-form-item label="截止时间" prop="deadline" :rules="[{ required: true, message: '请选择截止时间', trigger: 'blur' }]">
        <el-date-picker
        ...
        <!-- 作业内容 -->
        <el-form-item label="作业内容" prop="content" :rules="[{ required: true, message: '请输入作业内容', trigger: 'blur' }]">
            <el-input
  ...
  const form = ref({
  homeworkNum: '',
  deadline: null, // Ensure it starts as a proper Date object or null
  content: '',
});
  ...
  const beforeUpload = (file) => {
    const isValid = file.type === 'application/pdf' || file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || file.type === 'application/vnd.openxmlformats-officedocument.presentationml.presentation' || file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
    if (!isValid) {
  ...
  const submitHomework = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:8000/course/${courseId}/uploadHomework`, formData, {
        headers: {
  ...
  const fetchHomeworkList = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/teacher/course/${courseId}/homeworkList`);
